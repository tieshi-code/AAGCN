...
class Processor(IO):
        ...
        if self.arg.phase == 'train':
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.save_interval == 0) or (
                    ...
                    # Check if the subclass has the save_checkpoint method
                ...
                if ((epoch + 1) % self.arg.eval_interval == 0) or (
                    ...
                    # Adjust learning rate for ReduceLROnPlateau scheduler (after validation)
                    if hasattr(self, 'use_plateau_scheduler') and self.use_plateau_scheduler and hasattr(self, 'last_val_loss'):
                        self.adjust_lr(metric=self.last_val_loss)
                    
                    # Adjust learning rate for ReduceLROnPlateau scheduler (after validation)
                    if hasattr(self, 'use_plateau_scheduler') and self.use_plateau_scheduler and hasattr(self, 'last_val_loss'):
                        self.adjust_lr(metric=self.last_val_loss)
                    
                    # Early stopping check (if supported)
                    if hasattr(self.arg, 'early_stop_patience') and self.arg.early_stop_patience is not None:
                        if not hasattr(self, '_early_stop_init'):
                            # Initialize early stopping tracking on first evaluation
                            self._early_stop_init = True
                            self._best_val_acc = -1
                            self._early_stop_counter = 0
                            self._best_epoch = -1
                        
                        # Get validation accuracy (top-1)
                        if hasattr(self, 'result') and hasattr(self, 'label') and self.result is not None and self.label is not None:
                            result = np.array(self.result)
                            label = np.array(self.label)
                            rank = result.argsort()
                            hit_top_1 = [l == rank[i, -1] for i, l in enumerate(label)]
                            val_acc = sum(hit_top_1) * 100.0 / len(hit_top_1)
                            
                            if val_acc > self._best_val_acc:
                                self._best_val_acc = val_acc
                                self._best_epoch = epoch
                                self._early_stop_counter = 0
                                self.io.print_log('✓ New best validation accuracy: {:.2f}% at epoch {}'.format(self._best_val_acc, epoch + 1))
                            else:
                                self._early_stop_counter += 1
                                self.io.print_log('Validation accuracy: {:.2f}% (best: {:.2f}% at epoch {}) - Early stop counter: {}/{}'.format(
                                    val_acc, self._best_val_acc, self._best_epoch + 1, self._early_stop_counter, self.arg.early_stop_patience))
                            
                            if self._early_stop_counter >= self.arg.early_stop_patience:
                                self.io.print_log('Early stopping triggered! Best validation accuracy: {:.2f}% at epoch {}'.format(
                                    self._best_val_acc, self._best_epoch + 1))
                                break
        # test phase
            ...
            if self.arg.weights is None:
                raise ValueError('Specify --weights.')
    ...
    def get_parser(add_help=False):
        ...
        # optimizer parameters are defined in recognition.py
